...
def setup():
    ...
    parser.add_argument(
        ...
        "--setdefault",
        action="store_true",
        help="Set the database as the default database for Glycosylator (overwriting the current one in the process)",
        default=False,
    )
...
def main(args):
    db = gl.resources.PDBECompounds.from_file(args.input)
    if args.output is None:
        args.output = args.input + ".comp.pkl"
    if args.output != "None":
        db.save(args.output)
    if args.setdefault:
        gl.set_default_compounds(db, overwrite=True)
